...
export const ArticleAside = ({ categoryId, articleId }) => {
    ...
    return (
        <aside 
        className="h-full w-full bg-gray-200 p-4 rounded-lg shadow-md flex flex-col gap-4"
        ...
        >
            ...
            <ul className="flex lg:gap-4 gap-2 justify-center lg:flex-col md:flex-row md:flex-wrap flex-col">
                {articles?.map((article) => (
                <li key={article?.id}>
                    <Link 
                    to={`/articles/${article?.id}`} 
                    className="block bg-white border-1 border-l-8 border-l-primary border-gray-300 rounded-lg p-2 lg:p-4 hover:bg-gray-100 transition duration-300 cursor-pointer shadow-md focus:outline-none focus:ring-2 focus:ring-primary"
                    >
