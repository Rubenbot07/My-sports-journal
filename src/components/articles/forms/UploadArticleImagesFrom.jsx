...
export const UploadArticleImagesFrom = ({ articleId, articleSlug, category }) => {
  ...
  const inputs = [
    { name: "thumbnail", title: "Thumbnail Image (400x225)" },
    { name: "cover", title: "Cover Image (1200x675)" },
    { name: "banner", title: "Banner Image (1920x1080)" },
  ];
        ...
        {inputs.map((input) => (
          <div key={input.name} className="flex flex-col gap-3">
            <div className="flex justify-center w-1/2 mx-auto gap-2 items-center">
              ...
              <label
                htmlFor={input.name}
                className="bg-primary text-white px-4 py-2 w-1/2 min-w-72 rounded-full flex justify-between cursor-pointer"
              >
                {input.title}
                {files[input.name] && (
                  <CircleCheck className="text-white" size={25} aria-hidden="true" />
                )}
              </label>
            </div>
            ...
            {errors[input.name] && (
              <div
                id={`${input.name}-error`}
                className="text-primary text-sm flex items-center justify-center gap-2"
                role="alert"
              >
                <CircleX size={25} aria-hidden="true" />
          ...
          {Object.keys(files).length >= 3 && (
            <>
              <div className="text-primary text-sm flex items-center justify-center gap-2">
                ...
                <ListCheck size={30} aria-hidden="true" />
              </div>
              <Link
                to="/"
                className="bg-primary text-white px-4 py-2 w-1/2 min-w-72 rounded-full mx-auto"
              >
                Finish Upload
              </Link>
            </>
